...
function createWindow() {
  ...
  mainWindow
    ...
    .catch((error) => {
      ...
      mainWindow.loadURL(
        'data:text/html,<html><body style="font-family: Arial; padding: 24px;"><h2>Failed to load UI</h2><pre>' +
  ...
  const isSafeInternalUrl = (url) =>
    url.startsWith("file://") || url.startsWith("data:") || url === "about:blank";
    ...
    child.on("close", (code, signal) => {
      if (code !== 0) {
        const details =
          stderr ||
          stdout ||
          `python_api.py exited with ${code}${signal ? ` (signal ${signal})` : ""}`;
      ...
      try {
        ...
        if (
          result &&
          result.context &&
          Object.prototype.hasOwnProperty.call(result.context, "activePassword")
        ) {
        ...
        resolve({
          ok: false,
          status: 500,
          error: `Invalid JSON from python_api.py: ${String(e)}`,
          raw: stdout,
          stderr,
        });
      }
