...
CODE_MAPS = {
    ...
    'BG Check Status': [("None", "NONE"), ("Submitted", "SUB"), ("Cleared", "CLR")],
...
def _parse_sort_date(date_str: Optional[str]) -> datetime:
    """Parse a date string for sorting; invalid dates go to far future."""
    raw = (date_str or "").strip()
    if not raw:
        return datetime(9999, 12, 31)
    # Prefer dateutil if available for flexibility
    try:
        import importlib
        _dtparser = importlib.import_module('dateutil.parser')
        dt = _dtparser.parse(raw, dayfirst=False, yearfirst=False, fuzzy=True)
        return dt
    except Exception:
        pass
    # Fallback formats
...
def _try_parse_date(date_str: Optional[str]) -> Optional[datetime]:
    """Parse a date string; return None if invalid/empty."""
    raw = (date_str or "").strip()
    if not raw:
        return None
    try:
        import importlib
        _dtparser = importlib.import_module('dateutil.parser')
        return _dtparser.parse(raw, dayfirst=False, yearfirst=False, fuzzy=True)
    except Exception:
        pass
    for fmt in ("%m/%d/%Y", "%m/%d/%y", "%Y/%m/%d", "%m-%d-%Y", "%m-%d-%y"):
        try:
...
def create_kv_row(parent, label_text, value_text, lbl_style, val_style, bg=None):
    ...
    try:
        val_bg = val_style.get("bg", row.cget("bg"))
        val_fg = val_style.get("fg", CURRENT_PALETTE.get("fg_color", "#2c3e50"))
        val_font = val_style.get("font", FONTS["small"])
        ent = tk.Entry(
            row,
            bd=0,
            relief=tk.FLAT,
            ...
            readonlybackground=val_bg,
            font=val_font,
            highlightthickness=0,
        )
        ent.insert(0, value_text)
        ent.config(state="readonly")
        ent.pack(side=tk.LEFT, padx=(4, 0), fill=tk.X, expand=False)
    except Exception:
...
def build_info_bar(parent, person, fields, lbl_style, val_style):
    ...
    for label, key in fields:
        ...
        try:
            val_bg = val_style.get("bg", info_bar.cget("bg"))
            val_fg = val_style.get("fg", CURRENT_PALETTE.get("fg_color", "#2c3e50"))
            val_font = val_style.get("font", FONTS["small"])
            ent = tk.Entry(
                info_bar,
            ...
            ent.insert(0, person.get(key, "N/A"))
            ent.config(state="readonly")
            ent.pack(side=tk.LEFT, padx=(3, 12))
        except Exception:
...
def retint_foregrounds(container, old_palette, new_palette):
    """Update foreground colors that still match the old palette."""
    try:
        old_fg = old_palette.get("fg_color") if isinstance(old_palette, dict) else None
        new_fg = new_palette.get("fg_color") if isinstance(new_palette, dict) else None
        if not old_fg or not new_fg or old_fg == new_fg:
            return

        def _walk(widget):
            try:
                if isinstance(widget, (tk.Label, tk.LabelFrame, tk.Checkbutton, tk.Radiobutton, tk.Entry, tk.Text)):
                    try:
                    ...
                    if isinstance(widget, (tk.Checkbutton, tk.Radiobutton)):
                        try:
                            if widget.cget("activeforeground") == old_fg:
                                widget.configure(activeforeground=new_fg)
                        except Exception:
                            pass
                    if isinstance(widget, (tk.Entry, tk.Text)):
                        try:
                            if widget.cget("insertbackground") == old_fg:
                                widget.configure(insertbackground=new_fg)
                        except Exception:
                            pass
...
class WorkflowGUI:
    ...
    def toggle_theme(self):
        ...
        try:
            ...
            self._apply_palette()
            try:
                retint_foregrounds(self.root, old_pal, CURRENT_PALETTE)
                fix_checkbox_contrast(self.root)
            except Exception:
                pass
    ...
    def refresh_blocks(self) -> None:
        ...
        for person in self.people_data:
            ...
            if neo_date in date_cache:
                ...
                date_obj = _try_parse_date(neo_date)
                date_cache[neo_date] = date_obj
            is_scheduled = bool(date_obj)
            ...
            if is_scheduled and date_obj is not None:
        ...
        force_rebuild = bool(getattr(self, '_force_rebuild', False))
        if force_rebuild:
            try:
                ...
                pass
            try:
                self._person_widgets = {}
                self.card_registry = []
                if hasattr(self, '_left_header'):
                    delattr(self, '_left_header')
                if hasattr(self, '_right_header'):
                    delattr(self, '_right_header')
            except Exception:
                pass
            try:
                if unscheduled_display_persons:
            ...
            if unsched_has_more:
                try:
                    btn = pack_action_button(
                        self.left_col,
                        "Load more",
                        lambda: self._increase_render_limit("unscheduled"),
                        role="charcoal",
            ...
            if sched_has_more:
                try:
                    btn = pack_action_button(
                        self.right_col,
                        "Load more",
                        lambda: self._increase_render_limit("scheduled"),
                        role="charcoal",
                        font=FONTS["small_bold"],
                        width=12,
                        padx=4,
        ...
        try:
            ...
            for i, person in enumerate(desired_persons):
                ...
                for j in range(i + 1, len(desired_persons)):
                    nxt_pid = self._ensure_person_uid(desired_persons[j])
    ...
    def _on_person_saved(self, index: Optional[int], person: Dict[str, Any]) -> None:
        # Force a full rebuild so updated fields and styles always render
        try:
            self._render_epoch = getattr(self, '_render_epoch', 0) + 1
            self._last_render_context = None
            self._force_rebuild = True
